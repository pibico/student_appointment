...
class SchoolHolidaysList(Document):
	def validate(self):
		self.validate_days()
...
def get_events(start, end, filters=None):
	...
	if start:
		filters.append(['School Holidays', 'holiday_date', '>', getdate(start)])
	if end:
		filters.append(['School Holidays', 'holiday_date', '<', getdate(end)])

	return frappe.get_list('School Holidays List',
		fields=['name', '`tabSchool Holidays`.holiday_date', '`tabSchool Holidays`.description', '`tabSchool Holidays List`.color'],
		filters = filters,
		update={"allDay": 1})
